...
class DatabaseService:
    ...
    # Marker columns that record AI authorship on the product row itself, so
    # the catalog's "AI Filled" badge survives reloads (migration
    # 20260922010000_ai_filled_marker).
    _MARKER_COLUMNS = ("ai_filled_at", "ai_filled_columns")
    ...
    def write_ai_fill(
        ...
        """Write AI-extracted values and stamp the row as AI-filled.

        `ai_filled_columns` accumulates across fills (a second fill adds to the
        first). If the marker columns don't exist yet (backend deployed before
        the migration), the values are written without the stamp rather than
        failing the fill.
        """
        previous = (existing or {}).get("ai_filled_columns") or []
        stamped = {
            **updates,
            "ai_filled_at": datetime.now(timezone.utc).isoformat(),
            "ai_filled_columns": sorted(set(previous) | set(updates)),
        }
        try:
            return self.update_product(table_name, product_id, stamped)
        except Exception as e:
            if not any(col in str(e) for col in self._MARKER_COLUMNS):
                raise
            logger.warning(
                f"{table_name} has no AI-filled marker columns yet; "
                "writing values without the stamp"
            )
            return self.update_product(table_name, product_id, updates)
