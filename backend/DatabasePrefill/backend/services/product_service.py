...
class ProductService:
    ...
    async def batch_fill(
        ...
        `exclude_ids` are rows the caller already attempted earlier in the same
        multi-batch run (e.g. ones that failed), so the next batch moves on to
        new products instead of retrying the same failures."""
        # Get empty products, skipping any we've already filled in a prior run.
        already_done = self.tracker.processed_ids(table_name) | set(exclude_ids or [])
        products = self.database_service.get_empty_product_columns(
            table_name, limit, exclude_ids=already_done, owner_id=owner_id
        )
        usage_total = {"input_tokens": 0, "cached_tokens": 0, "output_tokens": 0, "thinking_tokens": 0}
        if not products:
            logger.info(f"No empty products found in {table_name}")
            return {"total": 0, "success": 0, "failed": 0,
                    "filled_ids": [], "details": [],
                    "model": self.llm_service.llm_model, "usage": usage_total}
        ...
        # the gaps in a product rather than re-describing it from zero, and the
        # answer is limited to that row's empty columns.
        logger.info(f"Analyzing {len(candidate_urls)} images...")
        analyses = await self.llm_service.batch_analyze(
            candidate_urls,
            existing_data=[self._context_for(p) for p in candidates],
            columns=[self._empty_columns(p) for p in candidates],
        )
        results = []
        usage_by_id: Dict[Any, Dict[str, int]] = {}
        for product, analysis in zip(candidates, analyses):
            usage = analysis.get("usage") or {}
            usage_by_id[product.get("id")] = usage
            for k in usage_total:
                usage_total[k] += int(usage.get(k, 0) or 0)
            results.append(analysis["data"] if "data" in analysis else analysis)
        ...
        # Per-product token usage, so the caller can price each item.
        for detail in details:
            if detail.get("id") in usage_by_id:
                detail["usage"] = usage_by_id[detail["id"]]
    ...
    def _empty_columns(self, product: Dict[str, Any]) -> List[str]:
        """The fillable columns this row is still missing - all the model is
        asked for, since anything else would be discarded on write anyway."""
        return [c for c in self._COLUMN_KEYS if is_empty_value(product.get(c))]
