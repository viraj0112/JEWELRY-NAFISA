...
class AuthContext:
    ...
    # Admin-set requests/minute for Gemini calls (None -> config default).
...
def _resolve_llm_settings(
    ...
    """Return (global_llm_api_key, default_model, requests_per_minute) from the
    admin settings row.

    The key is stored AES-GCM encrypted; `global_llm_api_key` is only present
    for rows written before the encryption migration. `select("*")` keeps this
    working whether or not newer columns (requests/minute) exist yet.
    """
    try:
        resp = (
        ...
        rpm = data.get("llm_requests_per_minute")
        return (
            key,
            (data.get("default_model") or settings.llm_model),
            int(rpm) if rpm is not None else None,
        )
    except Exception as e:
        logger.warning(f"Could not read llm_settings: {e}")
        return None, settings.llm_model, None
...
async def get_auth_context(
    ...
    global_key, default_model, requests_per_minute = _resolve_llm_settings(supabase)
