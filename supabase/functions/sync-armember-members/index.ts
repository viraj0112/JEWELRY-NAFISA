...
cron("0 * * * *", async () => {
  ...
  try {
    const plansResponse = await fetch(`${ARMEMBER_API_BASE_URL}arm_memberships?arm_api_key=${ARMEMBER_API_KEY}`);
    if (!plansResponse.ok) {
    ...
    for (const plan of plans) {
      ...
      const membersResponse = await fetch(`${ARMEMBER_API_BASE_URL}arm_plan_members?arm_api_key=${ARMEMBER_API_KEY}&arm_plan_id=${planId}`);
    ...
    const updates = Array.from(allMembers.entries()).map(async ([email, data]) => {
