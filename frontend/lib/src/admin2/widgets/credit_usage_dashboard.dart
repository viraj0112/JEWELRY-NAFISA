...
class CreditUsageDashboard extends StatefulWidget {
  const CreditUsageDashboard({super.key});
...
class _CreditUsageDashboardState extends State<CreditUsageDashboard> {
  ...
  Future<void> _fetchCreditData() async {
    ...
    try {
      ...
      final statsResponse =
          await supabase.rpc('get_credit_usage_stats').maybeSingle();
      ...
      if (mounted) {
        setState(() {
          // Basic stats
          // statsResponse is null when the RPC returns no row; the tiles
          // then show zeros rather than the whole card failing to render.
