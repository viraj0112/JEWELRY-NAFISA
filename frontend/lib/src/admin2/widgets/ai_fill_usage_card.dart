...
import '../../services/ai_fill_service.dart' show formatUsd;
...
class _UsageRow {
  ...
  /// Sum over runs whose model had a price configured.
  final double costUsd;

  /// Runs that used tokens but had no price (so [costUsd] is a lower bound).
  final int unpricedRuns;
}
    ...
    try {
      ...
      final rows = (res as List).map((r) {
        ...
        return _UsageRow(
          ...
          tokens: (r['total_tokens'] as num?)?.toInt() ?? 0,
          costUsd: (r['total_cost_usd'] as num?)?.toDouble() ?? 0,
          unpricedRuns: (r['unpriced_runs'] as num?)?.toInt() ?? 0,
        );
      }).toList();
      if (!mounted) return;
      setState(() {
              ...
              const Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 4),
                    Text(
                      'Who is filling catalog data with AI — runs, rows written and cost per user.',
        ...
        children: [
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  ...
                  '${r.tokens > 0 ? ' · ${_compact(r.tokens)} tokens' : ''}',
          ...
          SizedBox(
            ...
            child: Column(
              ...
              children: [
                Text(
                  r.tokens == 0 ? '-' : formatUsd(r.costUsd),
                  style: const TextStyle(
                      fontSize: 13, fontWeight: FontWeight.w700, color: _ink),
                ),
                Text(
                  r.unpricedRuns > 0 ? '+${r.unpricedRuns} unpriced' : 'cost',
                  style: TextStyle(
                      fontSize: 10,
                      color: r.unpricedRuns > 0
