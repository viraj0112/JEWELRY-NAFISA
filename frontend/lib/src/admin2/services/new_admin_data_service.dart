...
class NewAdminDataService {
  ...
  Future<void> moderateAsset({
    ...
    if (approve && assetResult != null) {
      try {
        ...
        debugPrint('Error updating product table after moderation: $e');
        rethrow;
      }
    }
    ...
    if (assetResult != null) {
      ...
      if (ownerId != null && ownerId.isNotEmpty) {
        try {
          ...
          debugPrint('Failed to send moderation notification to user: $e');
        }
      }
    }
  }
  ...
  Future<List<InventoryItem>> fetchContentActivityLog({
    ...
    Map<String, int> countBySource(List rows, bool useTable) {
    ...
    final likesMap = countBySource(likesRows, true);
    final viewsMap = countBySource(viewsRows, true);
    final sharesMap = countBySource(sharesRows, true);
    ...
    InventoryItem mapRow(dynamic row, String source) {
    ...
    var items = [
      ...productRows.map((r) => mapRow(r, 'products')),
      ...designerRows.map((r) => mapRow(r, 'designerproducts')),
      ...manufacturerRows.map((r) => mapRow(r, 'manufacturerproducts')),
    ];
  ...
  Future<List<MetalInsight>> _fetchMetalMetrics(String column) async {
    ...
    try {
      ...
      debugPrint('Error fetching metal metrics for $column: $e');
    }
