...
class _ContentSectionState extends State<ContentSection> {
  ...
  Future<void> _fetchCreditUsage() async {
    final statsFuture = supabase.rpc('get_credit_usage_stats').maybeSingle();
    final dailyUnlocksFuture = supabase.rpc('get_daily_unlocks');

    final results =
        await Future.wait<dynamic>([statsFuture, dailyUnlocksFuture]);

    // maybeSingle() yields null when the RPC returns no row; an empty map
    // keeps the downstream lookups working instead of throwing on the cast.
    _creditStatsData = (results[0] as Map<String, dynamic>?) ?? <String, dynamic>{};
  ...
  Future<void> _exportAllEngagementDataCsv(
    ...
    for (var product in products) {
      final itemId = product['id'].toString();
      const itemTable = 'products'; // You may need to track this
...
class MoreFiltersSheet extends StatefulWidget {
  ...
  const MoreFiltersSheet({super.key, this.onApply});
