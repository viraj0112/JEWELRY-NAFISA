...
import '../../widgets/ai_fill_progress_panel.dart';
import '../../widgets/llm_model_picker.dart';
import '../widgets/model_pricing_card.dart';
...
class _AdminAiFillScreenState extends State<AdminAiFillScreen> {
  ...
  final _batchSizeCtrl = TextEditingController();
  final _batchPauseCtrl = TextEditingController();
  final _rpmCtrl = TextEditingController();
  AiFillBatchSettings _batchSettings = const AiFillBatchSettings();
  AiFillProgress? _progress;
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      setState(() {
        ...
        _batchSettings = AiFillBatchSettings(
          batchSize: (settings?['fill_batch_size'] as num?)?.toInt() ?? 10,
          pauseSeconds:
              (settings?['fill_batch_pause_seconds'] as num?)?.toInt() ?? 10,
        );
        _batchSizeCtrl.text = '${_batchSettings.batchSize}';
        _batchPauseCtrl.text = '${_batchSettings.pauseSeconds}';
        _rpmCtrl.text =
            '${(settings?['llm_requests_per_minute'] as num?)?.toInt() ?? 10}';
        _credentials = creds;
        _users = users;
  ...
  Future<void> _saveSettings() async {
    ...
    final batchSize = whole(_batchSizeCtrl);
    final pause = whole(_batchPauseCtrl);
    final rpm = whole(_rpmCtrl);
    if (batchSize == null || batchSize < 1 || batchSize > 100) {
      _toast('Batch size must be a whole number from 1 to 100', error: true);
      return;
    }
    if (pause == null || pause < 0 || pause > 600) {
      _toast('Pause must be 0 to 600 seconds', error: true);
      return;
    }
    if (rpm == null || rpm < 0 || rpm > 10000) {
      _toast('Requests per minute must be 0 (no limit) to 10000', error: true);
      return;
    }
    try {
  ...
  Future<void> _run() async {
    ...
    try {
      final table = _table;
      // Large fills run in batches with pauses so Gemini's per-minute limits
      // (RPM/TPM) aren't hit; a small fill is simply one batch.
      final result = await _service.runInBatches(
        requested: _rowCount,
        settings: _batchSettings,
        runBatch: (limit, excludeIds) => _service.runAdminFill(
            tableName: table, limit: limit, excludeIds: excludeIds),
        onProgress: (p) {
      ...
      final reason = result.stoppedReason ?? result.firstErrorMessage;
      final cost = result.costLabel;
      _toast('Filled ${result.success}/${result.total} rows'
          '${cost == null ? '' : ' · $cost'}'
          '${reason == null ? '' : ' - $reason'}',
          error: result.failed > 0 && result.success == 0);
      _load();
    } catch (e) {
      if (!mounted) return;
      setState(() => _error = '$e');
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 16),
        ModelPricingCard(service: _service),
        const SizedBox(height: 16),
        _generateKeyCard(),
        const SizedBox(height: 16),
        _credentialsCard(),
        ...
        child: Column(
          ...
          children: [
            ...
            LlmModelPicker(
              ...
              keyController: _globalKeyCtrl,
              loadModels: (typedKey) =>
                  _service.listModels(llmApiKey: typedKey, global: true),
              label: 'Default model',
              decoration: _decoration('Default model'),
            ),
            const SizedBox(height: 16),
            const Text('Batching & rate limits',
                style: TextStyle(fontWeight: FontWeight.w700, color: _ink)),
            const SizedBox(height: 4),
            const Text(
              'Large fills are split into batches so Gemini\'s per-minute '
              'request/token limits (429 errors) aren\'t hit. Applies to all users.',
              style: TextStyle(fontSize: 12, color: _muted),
            ),
            const SizedBox(height: 10),
            Row(children: [
              Expanded(child: _numberField('Batch size', _batchSizeCtrl, '1-100')),
              const SizedBox(width: 10),
              Expanded(
                  child: _numberField('Pause (sec)', _batchPauseCtrl, '0-600')),
              const SizedBox(width: 10),
              Expanded(
                  child: _numberField('Requests/min', _rpmCtrl, '0 = no limit')),
            ]),
            const SizedBox(height: 16),
            Align(
  ...
  Widget _runCard() => _card(
        child: Column(
          ...
          children: [
            ...
            AiFillBatchHint(requested: _rowCount, settings: _batchSettings),
            const SizedBox(height: 12),
            if (_running) ...[
              AiFillProgressPanel(
                progress: _progress,
                requested: _rowCount,
                batchSize: _batchSettings.batchSize,
        ...
        child: Column(
          ...
          children: [
            Text(
                ...
                '${r.batches > 1 ? ' · ${r.batches} batches' : ''}',
                style:
                    const TextStyle(fontWeight: FontWeight.w600, color: _ink)),
            const SizedBox(height: 4),
            AiFillCostLine(result: r),
            if (r.stoppedReason != null) ...[
              const SizedBox(height: 4),
              Text('Stopped: ${r.stoppedReason}',
