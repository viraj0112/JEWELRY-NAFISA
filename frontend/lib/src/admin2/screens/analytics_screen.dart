...
class _AnalyticsScreenState extends State<AnalyticsScreen>
  ...
  Widget _buildGlobalResonanceSection(AnalyticsBundle bundle) {
    ...
    if (displayLocations.isEmpty) {
      displayLocations.add(const _CityData('NO DATA YET', 'Stable', 0.1));
    }

    // Pad to 5 exactly
    while (displayLocations.length < 5) {
      displayLocations.add(const _CityData('AWAITING DATA', 'Stable', 0.05));
    }
...
class _Card extends StatelessWidget {
  ...
  final Widget child;
  // Was `this.color = Colors.white` before `dart fix` dropped the unused
  // param; still read in build().
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (final entry in hotspots) {
      ...
      canvas.drawCircle(
          ...
          r * 1.8,
          Paint()
            ..color = gold.withValues(alpha: 0.06 * intensity)
      ...
      canvas.drawCircle(
          ...
          r,
          Paint()
            ..color = gold.withValues(alpha: 0.12 * intensity)
      ...
      canvas.drawCircle(
          ...
          r * 0.4,
          Paint()
            ..color = gold.withValues(alpha: 0.35 * intensity)
            ..style = PaintingStyle.fill);
      ...
      canvas.drawCircle(
          ...
          4.0 + intensity * 3.0,
          Paint()
            ..color = gold.withValues(alpha: 0.85 + intensity * 0.15)
            ..style = PaintingStyle.fill);
