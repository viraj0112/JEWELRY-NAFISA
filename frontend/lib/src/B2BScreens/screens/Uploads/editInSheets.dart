...
class _EditInSheetsDialogState extends State<EditInSheetsDialog> {
  static const _teal = B2BColors.primary;

  // Stone detail columns, in the same order as the bulk-upload template.
  static const List<String> _stoneColumns = [
    'Stone Weight',
    'Stone Type',
    'Stone Used',
    'Stone Setting',
  ...
  // always exported. Array-valued columns (see _arrayColumns) round-trip as
  // "a | b | c".
  ...
  // text[] columns in designerproducts/manufacturerproducts.
  static const Set<String> _arrayColumns = {
    'Images',
    'Category',
    'Metal Color',
    'Studded',
  ...
  Map<String, dynamic> _rowToProductData(List<dynamic> row) {
    ...
    for (var j = 0; j < _csvHeaders.length && j < row.length; j++) {
      ...
      if (_arrayColumns.contains(header)) {
      ...
      child: Column(
        ...
        children: [
          Text(
            ...
            style: const TextStyle(fontSize: 13, color: B2BColors.inkSoft),
          ),
          ...
          Wrap(
            ...
            children: _allColumns.map((c) {
              ...
              return FilterChip(
                ...
                selectedColor: _teal.withValues(alpha: 0.12),
          ...
          const Text(
            'Open the file in Excel / Google Sheets, edit values, then use "Import & Sync" to write changes back. Multi-value cells (Images, Category, Metal Color, Studded, Stone columns, Product Tags) export as ["a", "b"]; when editing you can also write a | b.',
            ...
            children: [
              ...
              if (_csvRows.isNotEmpty)
                Text('${_csvRows.length} data row(s)',
                    style:
                        const TextStyle(fontSize: 12.5, color: B2BColors.inkSoft)),
            ],
          ),
        ...
        if (snap.hasError) {
          return Padding(
            ...
            child: Text(
              ...
              style: const TextStyle(fontSize: 12.5, color: B2BColors.inkSoft),
            ),
          );
        }
        ...
        if (rows.isEmpty) {
          return const Center(
            child: Text('No syncs yet.',
                style: TextStyle(color: B2BColors.muted)),
          );
        }
          ...
          itemBuilder: (context, i) {
            ...
            return ListTile(
              ...
              leading: Icon(
                ...
                color:
                    (r['failed_count'] ?? 0) > 0 ? Colors.orange : B2BColors.primary,
              ),
              ...
              trailing: Text(
                '${r['created_at'] ?? ''}'
                    ...
                    .first
                    .replaceAll('T', ' '),
                style: const TextStyle(fontSize: 11, color: B2BColors.muted),
              ),
            );
          },
        );
