...
class _AiFillPageState extends State<AiFillPage> {
  static const _ink = B2BColors.ink;
  static const _accent = B2BColors.primary;
  static const _border = B2BColors.border;
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final batchSettings = await _service.getBatchSettings();
      if (!mounted) return;
      setState(() {
        _credential = cred;
  ...
  Future<void> _run() async {
    ...
    try {
      ...
      final result = await _service.runInBatches(
        ...
        runBatch: (limit, excludeIds) =>
            _service.runMyFill(limit: limit, excludeIds: excludeIds),
        onProgress: (p) {
          if (mounted) setState(() => _progress = p);
        },
        // Also stop if the sheet is closed mid-run.
              ...
              child: Column(
                ...
                children: [
                  ...
                  Row(children: [
                    Container(
                      ...
                      child: const Icon(Icons.auto_awesome,
                          color: B2BColors.gold, size: 20),
                    ),
                    const SizedBox(width: 12),
                    Text('AI Fill My Products', style: B2BText.serif(size: 23)),
                  ]),
                  const SizedBox(height: 6),
                  const Text(
                    'Auto-complete the empty details of YOUR products with AI. '
                    'Only your own products are processed.',
                    style: TextStyle(fontSize: 13, color: B2BColors.muted),
                  ),
                  ...
                  const Text(
                    ...
                    style: TextStyle(fontSize: 12, color: B2BColors.muted),
                  ),
                  ...
                  LlmModelPicker(
                    ...
                    keyController: _llmKeyCtrl,
                    loadModels: (typedKey) =>
                        _service.listModels(llmApiKey: typedKey),
                    label: 'Model',
                    defaultOptionLabel: 'Platform default (set by admin)',
                  ...
                  AiFillBatchHint(
                      requested: _rowCount, settings: _batchSettings),
  ...
  Widget _storedKeyHint() {
    ...
    return Row(children: [
      Icon(hint == null ? Icons.info_outline : Icons.lock_outline,
          size: 16, color: hint == null ? B2BColors.muted : _accent),
  ...
  Widget _keyStatusBanner() {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: ok ? B2BColors.successSoft : B2BColors.dangerSoft,
