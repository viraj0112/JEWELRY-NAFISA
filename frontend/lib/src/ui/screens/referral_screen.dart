...
class ReferralScreen extends StatefulWidget {
  const ReferralScreen({super.key, this.client});

  /// Supabase client to read referrals with. Production passes nothing and
  /// gets the global instance; tests inject a mock. Without this the screen
  /// reached for Supabase.instance directly, so referral_screen_test could
  /// wire up a mock client but the screen would never consult it.
...
class _ReferralScreenState extends State<ReferralScreen> {
  ...
  Future<List<Referral>> _fetchReferralHistory() async {
    final supabase = widget.client ?? Supabase.instance.client;
    ...
    try {
      ...
      // Deferred to after the frame. This method is kicked off from
      // initState(), so touching an inherited widget here - which
      // ScaffoldMessenger.of(context) does - throws
      // "dependOnInheritedWidgetOfExactType() was called before
      // initState() completed" whenever the fetch fails early. Post-frame is
      // also the first moment a ScaffoldMessenger above us is guaranteed to
      // exist.
