...
class BoardsProvider extends ChangeNotifier {
  ...
  Future<void> saveToBoard(int boardId, JewelryItem item) async {
    ...
    try {
      // Scoped to this user and capped at one row: `pins` has no unique
      // constraint on image_url, so an unscoped maybeSingle() threw as soon as
      // two people had pinned the same image.
      final existingPins = await _supabase
          .from('pins')
          .select('id')
          .eq('image_url', item.image)
          .eq('owner_id', user.id)
          .limit(1);
      final existingPin = existingPins.isEmpty
          ? null
          : existingPins.first;
