...
import 'package:jewelry_nafisa/src/auth/admin_guard.dart';
import 'package:jewelry_nafisa/src/auth/auth_gate.dart';
import 'package:jewelry_nafisa/src/auth/password_service.dart';
import 'package:jewelry_nafisa/src/auth/set_password_screen.dart';
...
final _router = GoRouter(
  ...
  routes: [
    GoRoute(
      path: '/admin',
      // AdminGuard, not MainScreen directly: the redirect above only proves a
      // session exists, and nothing inside admin2/ re-checks the role.
      builder: (context, state) => const AdminGuard(child: MainScreen()),
    ),
    GoRoute(
      path: '/',
...
Future<void> main() async {
  ...
  // A "reset password" email link signs the user in with a recovery event;
  // send them to the new-password form. The auth stream replays its latest
  // event, so a recovery handled during initialize() is still seen here.
  Supabase.instance.client.auth.onAuthStateChange.listen((data) {
    if (data.event == AuthChangeEvent.passwordRecovery) {
      PasswordService.recoveryPending = true;
      _router.go('/set-password');
